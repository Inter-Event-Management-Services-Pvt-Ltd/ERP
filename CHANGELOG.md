...
- Ran Phase 5 image scan evidence for Redis and Caddy: `redis:7-alpine` reports 0 critical / 0 high findings, while `caddy:2-alpine` and `caddy:2.11-alpine` still fail with critical/high OpenSSL, curl and Go stdlib findings; recorded OPEN-044 as the remaining public reverse-proxy image-scan blocker.
